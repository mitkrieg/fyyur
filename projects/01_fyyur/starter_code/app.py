...
def show_venue(venue_id):
    ...
    past_shows = [show for show in shows if show.start_time < dt.now()]
    upcoming_shows = [show for show in shows if show.start_time >= dt.now()]
...
def create_venue_submission():
    ...
    if valid:
        ...
        for entry, err in form.errors.items():
            flash("Need Valid Input for " + entry + " (. ERROR: " + err[0] + ")")
        return render_template("forms/new_venue.html", form=form)
...
def delete_venue(venue_id):
    ...
    try:
        print("DELETING VENUE id: ", venue_id)
        venue_delete = Venue.query.get(venue_id)
        db.session.delete(venue_delete)
        db.session.commit()
        body = {
            "success": True,
            "message": "Venue " + venue_delete.name + " was deleted.",
        }
    except:
        print("ERROR: ", sys.exc_info())
        db.session.rollback()
        body = {
            "success": False,
            "message": "Error in deleting Venue " + venue_delete.name,
        }
    finally:
        db.session.close()
...
def edit_artist(artist_id):
    ...
    form = ArtistForm(obj=artist_query, meta={"csrf": False})
...
def delete_artist(artist_id):
    # deletes an artist
    try:
        print("DELETING Artist id: ", artist_id)
        artist_delete = Artist.query.get(artist_id)
        body = {
            "success": True,
            "message": "Venue " + artist_delete.name + " was deleted.",
        }
        ...
        body = {
            "success": False,
            "message": "Error in deleting Venue " + artist_delete.name,
        }
        db.session.rollback()
    finally:
        db.session.close()
...
def create_show_submission():
    ...
    if valid:
        ...
        for entry, err in form.errors.items():
            flash("Invalid submission for " + entry + " ERROR: " + err)
        return render_template("forms/new_show.html", form=form)
